    ...
    var promiser = function(object, methodName, objectName) {
        var retfn = function () {
            ...
            args.push(function() {
                ...
                try {
console.log('success try ' + d);
var anArg;
for (anArg in arguments) console.log(' -> -> ' + anArg + ' : ' + arguments[anArg]);
...
console.log(err.message + ' -' + arguments + '- ');
            ...
            args.push(function() {
                ...
                try {
console.log('error try ' + d);
var anArg;
for (anArg in arguments) {
    console.log(' -> -> ' + anArg + ' : ' + JSON.stringify(arguments[anArg]));
}
...
//console.log('here is some merging possibly');
//console.log('or:' + JSON.stringify(oldRecord));
//console.log('r: ' + JSON.stringify(record));
//console.log('e: ' + JSON.stringify(_.extend(oldRecord || {}, record)));
//console.log('switch e: ' + JSON.stringify(_.extend(record, oldRecord || {})));

//what we actually want here is to merge based on merge types
//why is that not doing that?

//THIS,,,, is never refreshing. we could do with never refreshing some specific stuff,,,
//how about, never overwrite the updateable fields of __local__ records?
//i think we will do that...
//ideally we would be using the CACHE MODE here

//console.log(oldRecord.__local__);
//console.log(oldRecord.Meeting_Notes__c);

//console.log(record.__local__);
//console.log(record.Meeting_Notes__c);

// the cache doesnt have a cacheMode
// console.log(this.cacheMode);

//console.log('end of one merging');
                                
                                // no overwrite: oldest wins
                                //return _.extend(record || {}, oldRecord)
                                
                                // original: newest wins
                                  return _.extend(oldRecord || {}, record)
                                // so let the newest win and then we will see how that fares for syncing...
...
console.log('should be building query response?');
                return {
    ...
    Force.syncSObjectWithServer = function(method, sobjectType, id, attributes, fieldlist) {
        console.log("---> In Force.syncSObjectWithServer:method=" + method + " id=" + id + " attributes=" + attributes + " fieldlist=" + fieldlist);
...
console.log("Server Retrieve: " + sobjectType + ", " + id + ", " + fieldlist);
...
console.log('cacheMode:' + cacheMode);
        ...
        if (cache != null && cacheMode == Force.CACHE_MODE.CACHE_ONLY) {
console.log('cache query please');
            promise = fetchFromCache();

        } else {
console.log('server query please');
...
//DJB possible change: when saving cache, DONT overwrite locally modified record fields
                var cacheSaveAll = function(records) {
                    return cache.saveAll(records);
                };

//DJB possible change: when saving cacheForOriginals, overwrite all so that "original" continues to be "same as server" for later collision detection
...
console.log("config.type: " + config.type + ':' + cacheMode );
...
console.log('fetching remote objects from cache');
...
console.log('cache 2 : ' + options.cache);
                ...
                options.reset = true;
//console.log('fetch about to fetch');
//console.log('cache 3 : ' + cache);
...
//console.log('i think we fetched something : ' + resp + that.config.objectName);

//for (var a in resp.records) {
//for (var b in resp.records[a]) console.log(b + ': ' + resp.records[a][b]);
//}
                        return resp.records;
                    })
                    .done(options.success)
...
//djb
//for (var k in result)console.log("1514 " + k + ":" + result[k]);
//for (var k in result.attributes)console.log("1515 " + k + ":" + result.attributes[k]);
                
//djb this is dumb anyway - why are we looking in "result.attributes.type" when "result.type" works?
//djb first, change to result.type
//djb if that doesn't work, change to "best out of result.type and result.attributes.type"
//                    var sobjectType = result.attributes.type;
//djb should really be extending Force.SObjectCollection FIXME
                    //var sobjectType = result.type;
//djb *maybe* this didnt work, because the Meeting__c on sync is missing its type so try
                    var sobjectType = result.attributes.type;
                    
                    if (typeof sobjectType === "undefined") sobjectType = result.type;
//djb that seems to work. I *may* have to go with the suggestion made on the forum and manipulate the
//djb object attributes directly instead of relying on set to do it, cos set dont do it.
